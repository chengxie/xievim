...
call <SID>AddAlternateExtensionMapping('h',"cpp,c,cxx,cc,CC,m,mm")
call <SID>AddAlternateExtensionMapping('H',"CPP,C,CXX,CC,M,MM")
  ...
  if (bufNr == -1)
     ...
     " Buffer did not exist....create it
	  if !filereadable(FILENAME)
		  if input(fnamemodify(FILENAME, ":t")." not exist! Do you want to create it? [yes/no]") != "yes"
			  echo ''
			  return
		  else
			  echo ''
     ...
     if (bufWindow == -1) 
        ...
        elseif (splitType == "t")
           ...
           silent! execute ":b!".bang." " . FILENAME
        endif
        if (v:errmsg != "")
           echo v:errmsg
        endif
