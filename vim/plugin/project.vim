...
function! s:Project(filename) " <<<
	...
	if !exists('s:proj_foldcolumn')
		let s:proj_foldcolumn = 0
	endif
	if !exists('g:proj_close_fold_onstart')
		let g:proj_close_fold_onstart = 0
	endif
    ...
    if !exists("g:proj_running") || (bufwinnr(g:proj_running) == -1) " Open the Project Window
		...
		if exists("g:proj_running")
			"这里有可能当前目录不是project文件的路径，但bufname取出来是相对路径
...
"            silent! bnext
	...
	function! s:Connect_To_Cscope(path)
		let tagsfile = a:path . "/tags"
		let csfile = a:path . "/cscope.files"
		let csoutfile = a:path . "/cscope.out"
		...
		if filereadable(csfile)
			let csfilesdeleted = delete(csfile)
			if (csfilesdeleted != 0)
		...
		" find ~/.third_party/include -name '*.h' -o -name '*.hpp' -o -name '*.c' -o -name '*.cc' -o -name '*.cpp' > third_party.files
		silent! execute "!find " . a:path . " -name '*.h' -o -name '*.hpp' -o -name '*.c' -o -name '*.cc' -o -name '*.cpp' > " . csfile

		if filereadable(csoutfile)
			let csoutdeleted = delete(csoutfile)
			if(csoutdeleted != 0)
		...
		if (executable('ctags'))
			silent! execute '!ctags -R -h ".c.cc.cpp.h.hpp" --c++-kinds=+p --fields=+ialS --extra=+q -L ' . csfile . '  -o '. tagsfile
		endif
		if (executable('cscope') && has("cscope"))	
			" cscope -b -i xx.files -f xx.csout
			silent! execute "!cscope -b -i " . csfile . " -f " . csoutfile
			execute "normal :"
			if filereadable(csoutfile)
				execute "cs add " . csoutfile
			endif
			if filereadable(g:gcc_cs)
				execute "cs add " . g:gcc_cs
			endif
		endif
	endfunction

	function! s:MakeCtagsCscope()
		let home = ""
		let lineno = 1
		while (lineno <= line('$'))
			if getline(lineno) =~ '{'	
				let infoline = s:RecursivelyConstructDirectives(lineno)
		...
		if home != "/*" && home != ""
			if has("cscope")
				call s:Connect_To_Cscope(home)
			else
				silent! execute 'ctags -R -h ".c.cpp.h" --c++-kinds=+p --fields=+ialS --extra=+q -o ' . home . '/tags'
			endif
		endif
	endfunction

	"切换显示行号
	function! s:ShowNumber()
		if getbufvar(winbufnr('$'), '&number') == 1
			silent! execute ':set nonumber'
		else
			silent! execute ':set number'
		endif
	endfunction
	...
	function! s:FindWindowByName(name)
		...
		let find_win = -1
		while i < win_count
			if bufname(winbufnr(i + 1)) == a:name
				let find_win = i + 1
				break
			endif
			let i = i + 1
	...
	function! s:GotoSelfWin()
		let selfwin = bufwinnr(g:proj_running)
		if selfwin != -1
			exe selfwin . "wincmd w"
		endif
		return selfwin
	endfunction
	...
	function! s:OnBufEnter()
		...
		let pre_win = winnr()
		if s:GotoSelfWin() != -1
			let lineno = s:FindPathLine(path)
			if lineno != -1
				call cursor(lineno, 1)
				let name = '\<'.name.'\>'
				call search(name)
				call s:Window_Highlight_Cursor_Line()	
				call s:Window_Highlight_Line()
				while foldclosed(line('.')) != -1
					normal! za
				endwhile
				call winline()
			endif
			exe pre_win . 'wincmd w'
		endif
	endfunction
	...
	let s:CursorLineHighlightIdx = 0
	function! s:Window_Highlight_Cursor_Line()
		if s:CursorLineHighlightIdx > 0
			call matchdelete(s:CursorLineHighlightIdx)
			let s:CursorLineHighlightIdx = 0
		endif
		if getbufvar(bufnr('%'), "&buftype") != ''
			...
			let s:CursorLineHighlightIdx = matchadd('projectCurCursorLine', pat)
			call search(pat)
		endif
	endfunction

	" Highlight the current line
	function! s:Window_Highlight_Line()
		" Clear previously selected name
		match none

		" Highlight the current line
		let pat = '/\%' . line('.') . 'l\s\+\zs.*/'
		exe 'match projectCurEditFileName ' . pat
	endfunction

	function! s:StartEditTheProjectFile()
		if s:GotoSelfWin() == -1
			echo "ERROR! Not found window for project."
			return -1
		endif
		if getbufvar(bufnr('%'), "&buftype") == ''
			return 0
		...
		setlocal modifiable
		call setbufvar(bufnr('%'), "&buftype", "")
		match none
		return 0
	endfunction
	
	function! s:EndEditTheProjectFile()
		if s:GotoSelfWin() == -1
			return -1
		endif
		if getbufvar(bufnr('%'), "&buftype") != ''
	...
	function! s:Create(recursive)
		if s:StartEditTheProjectFile() == -1
			return	
		endif
		call s:CreateEntriesFromDir(a:recursive)
		call s:EndEditTheProjectFile()
		call s:RefreshFoldColumn()
		call s:MakeCtagsCscope()
	endfunction

	function! s:Refresh(recursive)
		if s:StartEditTheProjectFile() == -1
			return
		endif
		call s:RefreshEntriesFromDir(a:recursive)	
		call s:EndEditTheProjectFile()	
		call s:RefreshFoldColumn()
		call s:MakeCtagsCscope()
	endfunction
	
	function! s:RefreshFoldColumn()
		let s:proj_foldcolumn = 0
		let curline = 0
		let licount = line('$')
		while curline < licount
			let fl = foldlevel(curline + 1)
			if fl > s:proj_foldcolumn
				let s:proj_foldcolumn = fl
			endif
			let curline = curline + 1
		endwhile
		let s:proj_foldcolumn = s:proj_foldcolumn + 1
		call s:DoSetup()
	endfunction
    ...
    function! s:DoSetup()
        setlocal foldenable foldmethod=marker foldmarker={,} commentstring=%s nonumber noswapfile shiftwidth=1
		silent! exe 'setlocal foldcolumn='. s:proj_foldcolumn
    ...
    if match(g:proj_flags, '\Cs')!=-1 && has('syntax') && exists('g:syntax_on') && !has('syntax_items')
		...
		highlight def link projectCurCursorLine Search
        if hlexists('MyProjectCurEditFileName')
            highlight link projectCurEditFileName MyProjectCurEditFileName
        else
            highlight def link projectCurEditFileName Search
        endif
    ...
    function! s:DoFoldOrOpenEntry(cmd0, cmd1)
        if getline('.')=~'{\|}' || foldclosed('.') != -1
			...
			if (match(g:proj_flags, '\Cc') != -1)
				...
				"如果有MiniBufExplorer的窗体，就跳转到自己的窗口，再跳回去，可以恢复meb窗体的尺寸
				let mbe_win = s:FindWindowByName('-MiniBufExplorer-')
				if mbe_win != -1
					call s:GotoSelfWin()
					let edit_win = s:GotoEditWin()
					if edit_win == -1
    ...
    if !exists("g:proj_running")
        ...
        " Mappings <<<
		nnoremap <buffer> <silent> <Tab> :call <SID>OnTab()<CR>
		nnoremap <buffer> <silent> = :silent! foldopen<CR>
		nnoremap <buffer> <silent> - :silent! foldclose<CR>
		nnoremap <buffer> <silent> + :silent! %foldopen!<CR>
		nnoremap <buffer> <silent> _ :silent! %foldclose<CR>
        ...
        nnoremap <buffer> <silent> a  \|:call <SID>ShowNumber()<CR>
        ...
        nnoremap <buffer>          <LocalLeader>F0 \|:call <SID>ListSpawn("_fold")<CR>
        nnoremap <buffer> <silent> <LocalLeader>c :call <SID>Create(0)<CR>
        nnoremap <buffer> <silent> <LocalLeader>C :call <SID>Create(1)<CR>
        nnoremap <buffer> <silent> <LocalLeader>r :call <SID>Refresh(0)<CR>
        nnoremap <buffer> <silent> <LocalLeader>R :call <SID>Refresh(1)<CR>
		...
		exec 'au CursorMoved '.bufname.' call s:OnCursorMoved()'
	...
	if !exists(':PE')
		command! PE call <SID>StartEditTheProjectFile()
	endif

	if !exists(':PW')
		command! PW call <SID>EndEditTheProjectFile()
	endif

	if !exists(':PC')
		command! PC call <SID>MakeCtagsCscope()
	endif

	if !exists(':Pcreate')
		command! Pcreate call <SID>Create(1)
	endif
endfunction " >>>
...
if !exists("*<SID>DoToggleProject()") "<<<
    function! s:DoToggleProject()
		if !exists('g:proj_running')
			let name = input("Enter the filename for your project: ", "", "file")
			if strlen(name) == 0
				return
			endif
			...
			if filereadable(name)
				call s:Project(name)				
				call s:MakeCtagsCscope()
				if g:proj_close_fold_onstart == 1
					call s:CloseFold()
				endif
				call s:RefreshFoldColumn()
			else
				if getftype(name) == "dir"
					echo 'error ! "'.name.'" is direct! '
					return
				else
					call s:Project(name)
					call s:Create(1)	
