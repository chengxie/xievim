...
if !exists('loaded_taglist')
	...
	let s:TagList_CursorLineMatchIdx = 0
...
" Object-C language
let s:tlist_def_objc_settings = 'objc;i:interface;c:class;m:method;p:property'
...
function! s:Tlist_Window_Display_Help()
    ...
    if s:tlist_brief_help
        ...
        call append(11, '" _ : Close all folds')
...
function! s:GotoSelfWin()
	let selfwin = bufwinnr(g:TagList_title)
	if selfwin != -1
...
function! s:OnCursorMoved()
	call s:Tlist_Window_Highlight_Cursor_Line()
endfunction

"Highlight the current cursor line
function! s:Tlist_Window_Highlight_Cursor_Line()
	if s:TagList_CursorLineMatchIdx > 0
		call matchdelete(s:TagList_CursorLineMatchIdx)
	endif
	let pat = '\%' . line('.') . 'l\s\+\zs.*'
	if getline(line('.'))[0] != " "
		let pat = '\%' . line('.') . 'l\+\zs.*'
	endif
	let s:TagList_CursorLineMatchIdx = matchadd('TagListCurCursorLine', pat)
	call search(pat)
endfunction
...
"判断是否没有编辑窗口，如果没有编辑窗口了，就退出VIM
function! s:NoEditWinToExit()
	if s:FindEditWindow() == -1
		quit
	endif
endfunction
...
function! s:Tlist_Window_Init()
    ...
    if has('gui_running') || &t_Co > 2
		...
		highlight def link TagListCurCursorLine Search		
    ...
    inoremap <buffer> <silent> +             <C-o>:silent! %foldopen!<CR>
    inoremap <buffer> <silent> _             <C-o>:silent! %foldclose<CR>
    ...
    augroup TagListAutoCmds
		...
		if g:Tlist_Exit_OnlyWindow
	 	    autocmd BufEnter __Tag_List__ nested
				\ call s:NoEditWinToExit()
		endif
		autocmd BufWinEnter * call s:Tlist_Window_Check_Width()
		...
		autocmd CursorMoved __Tag_List__ call s:OnCursorMoved()
    augroup end
...
function! s:Tlist_Window_Open()
	...
	"如果当前窗口是MiniBufExplorer，就将光标切换到编辑窗口
